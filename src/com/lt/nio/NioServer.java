...
public class NioServer {
	...
	public NioServer init(String ip,int port) throws IOException{
        //获取一个ServerSocket通道
    ...
    public void listen(){
        ...
        while(true){
            ...
            try {
				...
				try {
					SocketChannel channel = (SocketChannel)key.channel(); 
					String ip=channel.getRemoteAddress()+"";
					String prevLog=keysMap.get(ip)==null?ip:keysMap.get(ip);
					logger.info(prevLog+ " - 关闭连接 - "+e.getMessage());
                	...
                	logger.info("关闭失败"+e2.getMessage());
                	e2.printStackTrace();
                }
       ...
       //第二通道  
        if(Integer.parseInt(dataList.get(36), 16)==1){
            ...
            if(map.containsKey(address)){ 
            	map.get(address)[0]=sign+data1+data2+"."+data3;
            }
            else{
            	String [] temp=new String[4];
            	temp[0]=sign+data1+data2+"."+data3;
        ...
        if(Integer.parseInt(dataList.get(30), 16)==2){
            ...
            if(map.containsKey(address)){ 
            	map.get(address)[1]=sign+data1+data2+"."+data3;
            }
            else{
            	String [] temp=new String[4];
            	temp[1]=sign+data1+data2+"."+data3;
            	map.put(address, temp);
            }
            //param.add("空气湿度");
            //param.add(sign+data1+data2+"."+data3);
            ...
            if(map.containsKey(address)){ 
            	map.get(address)[2]=sign+data1+data2+"."+data3;
            }
            else{
            	String [] temp=new String[4];
            	temp[2]=sign+data1+data2+"."+data3;
            	map.put(address, temp);
            }
            //param.add("二氧化碳浓度");
            //param.add(sign+data1+data2+"."+data3);
            ...
            if(map.containsKey(address)){ 
            	map.get(address)[3]=sign+data1+data2+data3+"00";
            }
            else{
            	String [] temp=new String[4];
            	temp[3]=sign+data1+data2+data3+"00";
            	map.put(address, temp);
            }
        ...
        if(keysMap.containsKey(address)&&map.containsKey(address)){
        	String[] temp=map.get(address);
        	if(temp[0]!=null&&temp[1]!=null&&temp[2]!=null&&temp[3]!=null){
            	...
            	param.add("氨气浓度");
                param.add(temp[3]);
            	insertForKey(address,keysMap.get(address),param);	
            	
            	keysMap.remove(address);
            	map.remove(address);
        	}
    ...
    private static void insertForKey(String address,String deviceId,List<Object> param){
    	...
    	if(deviceList.size()>0){
        	...
        	logger.info("设备("+deviceId+")插入数据，主键："+result);
    	}
    	else{
    		logger.info("系统中未查询到设备("+deviceId+")，IP地址："+address);
    	}
    }
    /**
     * 输出数据
    ...
    public static void main(String[] args) throws IOException {
        ...
        new NioServer().init(ip,port).listen();
